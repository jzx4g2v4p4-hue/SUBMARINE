...
const FOCUS_PRESETS = {
  ...
  sleep:  { carrier: 160, beat: 4,   label: 'Theta — deep relaxation, sleep onset' },
...
const APP = {
  ...
  /* Playback sources */
  musicSrc:     null,
  subSrcs:      [],   // [BufferSourceNode x3]
  focusSrcs:    [],   // [left, right] oscillators

  /* Buffers */
  musicBuffer:  null,
  subBuffers:   [],   // [AudioBuffer x3]
...
function ensureCtx() {
  ...
  // Master chain: layerGains → subBusGain ──┐
  //               musicGain               ──┤→ limiter → masterGain → destination
  //               focusGain               ──┘
  ...
  // Per-layer gain nodes (route → subBusGain)
...
function playAudio(seekOffset) {
  ...
  for (let i = 0; i < NUM_LAYERS; i++) {
    ...
    src.buffer = buf;
    src.loop = true;
    src.playbackRate.value = 1; // speed already baked into buffer
...
/*
  Generates an audio buffer encoding the affirmations as one of several
  masking textures using Web Audio API math. No TTS — all synthesis.

  Textures:
    tone    - FM tone bursts per word (v1 style, refined)
    whisper - shaped white noise bursts (simulated whisper envelope)
    deep    - low-frequency rumble with amplitude modulation
    hiss    - high-pass filtered noise panning across stereo
    pulse   - rhythmic bandpass pulse bed
*/
function generateSubBuffer(ctx, durationSec, layer, affirmations) {
  ...
  let affIdx = 0;
  let iterations = 0;
  const maxIter = affirmations.length * density * 4;
...
function writeWhisper(L, R, s0, len, sr, hash, affIdx) {
  // Bandpass noise burst — simulates a whisper envelope
  const fc  = 2000 + (hash % 2000);  // center freq
...
function writeDeepMask(L, R, s0, len, sr, hash, affIdx) {
  // Low rumble with amplitude modulation
...
function writeHiss(L, R, s0, len, sr, hash, affIdx) {
  // High-pass noise with stereo pan sweep
  const vol = 0.10;
  let x1=0, x2=0, y1l=0, y2l=0, y1r=0, y2r=0;
...
function writePulse(L, R, s0, len, sr, hash, affIdx) {
  // Rhythmic bandpass tone pulse
  const freq = 200 + (hash % 400);
  const pulseRate = 4 + (hash % 8); // Hz
  const vol = 0.16;
  for (let i = 0; i < len; i++) {
    const lt = i/sr;
    const e  = env(i, len);
...
function startFocusLayer() {
  ...
  stopFocusLayer();
  const p = FOCUS_PRESETS[APP.focusPreset] || FOCUS_PRESETS.focus;
  const carrierL = p.carrier;
  const carrierR = p.carrier + p.beat;
  const gain = APP.focusVol;
  ...
  gL.gain.value = gain * 0.5;
  gR.gain.value = gain * 0.5;
  ...
  if (panL && panR) {
    ...
    // Fallback: no panner, just mix both tones mono
...
/* ── TOGGLE PILL SYNC (Safari < 15.4 fallback) ─────── */
...
function loadSession() {
  try {
    ...
    APP.focusPreset = data.focusPreset || 'focus';
...
async function doExport() {
  ...
  try {
    ...
    // Chain: musicGain + layerGains → subBus → limiter → masterGain → destination
    ...
    // Focus layer (baked in)
    if (APP.focusEnabled) {
      const fp = FOCUS_PRESETS[APP.focusPreset] || FOCUS_PRESETS.focus;
...
function normalize(audioBuffer) {
  ...
  if (peak < 0.001 || peak >= 0.98) return; // already fine or too quiet
...
(function init() {
  ...
  // Auto-resume audio context on tap (iOS requirement)
